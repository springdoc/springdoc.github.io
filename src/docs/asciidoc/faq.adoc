...
For example let's consider a `PersonDTO` class with an `workAddress` and `homeAddress` attribute of type `Address`:
...
For example let's consider a `PersonDTO` class with an `email` attribute:
...
=== Why i get `no API definition provided` when navigating to `/swagger-ui/index.html` (first thing) ?

You may navigate directly `/swagger-ui/index.html` but you will get the Error message.
Since `v1.6.0` query params are disabled by default for link:https://github.com/swagger-api/swagger-ui/security/advisories/GHSA-qrmm-w75w-3wpx[security reasons, window="_blank"].
This is the default setting, since link:https://github.com/swagger-api/swagger-ui/releases/tag/v4.1.3[swagger-ui v1.4.3, window="_blank"].

You should only use the following link:  `\http://serverName:applicationPort/${springdoc.swagger-ui.path}`
...
|===
|springdoc-openapi Versions | spring-boot Versions
|`1.6.0` + | `2.6.x`, `1.5.x`
|`1.5.9` + | `2.5.x`, `1.5.x`
|`1.5.0` + | `2.4.x`, `1.5.x`
|`1.4.0` + | `2.3.x`, `1.5.x`
