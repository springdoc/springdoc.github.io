...
=== General overview
image::img/common.webp[springdoc-openapi v2 module architecture overview,1200,458]
...
----
   <dependency>
      ...
      <version>{springdoc-version2}</version>
   </dependency>
----

NOTE: This dependency is relevant if you want to generate the OpenAPI description without using the swagger-ui or Scalar.
...
NOTE: Since `v2.9.1`, the Scalar starters no longer register a `ForwardedHeaderFilter` (Web MVC) or a `ForwardedHeaderTransformer` (WebFlux). Adding the starter therefore no longer makes the whole application trust the `Forwarded` and `X-Forwarded-*` headers of every caller. If your application runs behind a trusted reverse proxy and the Scalar page must be rendered with the external host, enable Spring Boot's own support instead:
...
To expose the swagger-ui and Scalar, on the management port, you should set
...
management.endpoints.web.exposure.include=openapi, scalar
----
...
. Or An Endpoint, that routes to the scalar:

- `\http://serverName:managementPort/actuator/scalar`
...
For the example, you should also be able to see the springdoc-openapi endpoints depending on the chosen UI:

- `\http://serverName:9090/actuator`
- `\http://serverName:9090/actuator/swagger-ui` or `\http://serverName:9090/actuator/scalar`
...
TIP: If you want to reach the application endpoints, from the swagger-ui/scalar deployed beneath the actuator base path, using a different port from your application, `CORS for your endpoints` on your application level should be enabled.

Additionally, it is also possible to combine this property, with the existing property to display the actuator endpoints in the swagger-ui/scalar.
...
The scalar UI will be then accessible through the actuator port:
...
- The scalar UI will be then accessible through the application port. For example: `\http://serverName:applicationPort/scalar`
- A dedicated group for the actuator endpoints will be by default added.
- If no group is defined for the application, a default one will be added.
