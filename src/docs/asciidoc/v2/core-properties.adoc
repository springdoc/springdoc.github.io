...
= springdoc-openapi Core Properties Reference (v2)
:toc: left
:toclevels: 3
:sectanchors:
:icons: font
...
|springdoc.cache.max-entries | `100` | `Integer`. The maximum number of OpenAPI descriptions kept in the cache. One entry is cached per requested locale (`Accept-Language`); the oldest entry is evicted once the limit is reached.
