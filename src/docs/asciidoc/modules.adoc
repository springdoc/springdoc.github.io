...
=== General overview
image::img/archi-springdoc.webp[springdoc-openapi module architecture overview,1600,760]
...
`springdoc-openapi` provides MCP (Model Context Protocol) integration modules that automatically expose your REST API endpoints as AI tools. For detailed usage and configuration, see the <<mcp>> section.

*   If your project is using `spring-boot-starter-web` and you need MCP support, add the following dependency:
...
*   If your project is using `spring-boot-starter-webflux` and you need MCP support, add the following dependency:
