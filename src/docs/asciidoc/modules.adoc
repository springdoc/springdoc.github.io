...
The projects that use `spring-data-rest` and need the support for `Pageable` type can  add the following dependency in combination with the `springdoc-openapi-ui` dependency.
